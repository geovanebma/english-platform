...
function buildBadges(progress) {
  ...
  return [
    ...
    { id: "vocab50", label: "Vocabulário 50", unlocked: learned >= 50, description: "Marque 50 palavras learned" },
    { id: "call5", label: "Fala ativa", unlocked: sessions >= 5, description: "Faça 5 sessões com nativos" },
    { id: "test3", label: "Monitoramento", unlocked: tests >= 3, description: "Complete 3 testes de nível" },
  ];
}
...
export default function Profile({ setCurrentView, color = "#4f7cff" }) {
  ...
  const recentActivity = useMemo(() => {
    ...
    if (modules.speak_with_natives?.sessions_history?.length) {
      ...
      rows.push({ label: `Sessão com nativo (${last.native_name || "Nativo"})`, date: last.started_at });
    }
    if (modules.test_english_level?.last_result?.finished_at) {
      rows.push({ label: "Teste de nível concluído", date: modules.test_english_level.last_result.finished_at });
    }
    return rows
    ...
    <section className="profile-shell" style={{ "--profile-theme": color }}>
      ...
      <section className="profile-hero-card">
        ...
        <div className="profile-stat-chips">
          ...
          <span><Trophy size={14} /> {totalCompletedModules} módulos ativos</span>
      ...
      <section className="profile-grid">
        ...
        <article className="profile-card">
          ...
          <div className="profile-summary-grid">
            <div>
              <Clock3 size={15} />
              <span>Meta diária: {form.daily_goal_minutes} min</span>
            </div>
            <div>
              <Gem size={15} />
