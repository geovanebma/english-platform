...
const TRANSLATION_ITEMS = [
  { id: "t1", level: 1, from: "en", source: "Good morning", target: "Bom dia", accepted: ["Bom dia!"] },
  {
    id: "t2",
    level: 1,
    from: "pt",
  ...
  { id: "t3", level: 1, from: "en", source: "I am ready", target: "Eu estou pronto", accepted: ["Estou pronto", "Eu to pronto"] },
  { id: "t4", level: 2, from: "pt", source: "Eu preciso de ajuda", target: "I need help", accepted: ["I need some help", "I need your help"] },
  { id: "t5", level: 2, from: "en", source: "She studies every day", target: "Ela estuda todos os dias", accepted: ["Ela estuda diariamente"] },
  { id: "t6", level: 2, from: "pt", source: "Nos chegamos cedo", target: "We arrived early", accepted: ["We got here early", "We came early"] },
  {
    id: "t7",
    level: 3,
    from: "en",
  ...
  { id: "t8", level: 3, from: "pt", source: "Eu vou confirmar os detalhes depois", target: "I will confirm the details later", accepted: ["I'll confirm the details later"] },
  { id: "t9", level: 3, from: "en", source: "Their presentation was very clear", target: "A apresentacao deles foi muito clara", accepted: ["A apresentacao deles estava muito clara"] },
  {
    id: "t10",
    level: 4,
    from: "pt",
...
function evaluateTranslation(item, input) {
  ...
  if (normalizedVariants.includes(answerNorm)) {
    return {
      correct: true,
      message: item.contextHint || "Correct. Nice translation.",
  ...
  if (bestSimilarity >= 0.82) {
    return {
      correct: true,
      message: "Very close and equivalent by context. Considered correct.",
      acceptedVariants: variants,
    };
  }
  ...
  return {
    correct: false,
    message: `Expected answer: "${item.target}"`,
    acceptedVariants: variants,
  };
}
        ...
        >
          {active && unlocked ? (
            <div className="translation-tooltip-parent" aria-hidden="true">
              <div className="translation-tooltip">COMECAR</div>
            </div>
          ) : null}
          <span className="translation-node-icon">
            <img src={iconSrc} alt="" className="translation-node-imgicon" />
          </span>
        </button>
      </div>
...
export default function TranslationPractice({ setCurrentView, color = "#573a22" }) {
  ...
  const verify = async () => {
    ...
    const variantsLabel = result.acceptedVariants.length > 1
      ? ` Accepted variants: ${result.acceptedVariants.map((v) => `"${v}"`).join(" | ")}`
      : "";
    ...
    <section className="translation-page-shell" style={{ "--translation-theme": color }}>
      <header className="translation-header-card">
        ...
        <div className="translation-header-main">
          <div className="translation-kicker">TRANSLATION TRAIL</div>
          <h1>Current translation content: A1</h1>
        </div>
      ...
      {loading ? (
        ...
        <>
          <div className="translation-trail-summary">Progress: {completedIds.size}/{total} exercises completed</div>
