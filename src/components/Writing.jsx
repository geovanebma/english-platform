...
const WRITING_AI_ENDPOINT = import.meta.env.VITE_WRITING_API_URL || "";
...
async function fetchWritingAiFeedback(exercise, text) {
  if (!WRITING_AI_ENDPOINT) return null;
  try {
    const res = await fetch(WRITING_AI_ENDPOINT, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ exercise, text }),
    });
        ...
        >
          {active && unlocked ? (
            <div className="writing-tooltip-parent" aria-hidden="true">
              <div className="writing-tooltip">{getUiLabel("writing.start", "START")}</div>
            </div>
          ) : null}
    ...
    <section className="writing-page-shell" style={{ "--writing-theme": color }}>
      <header className="writing-header-card">
        ...
        <div className="writing-header-main">
          <div className="writing-kicker">{getUiLabel("writing.kicker", "A1 - WRITING TRAIL")}</div>
          <h1>
            {getUiLabel("writing.current_content", "Current writing content: {level}").replace(
              "{level}",
              "A1"
            )}
          </h1>
      ...
      {view === "trail" ? (
        <>
          <div className="writing-trail-summary">
            {getUiLabel("writing.progress_summary", "Progress: {done}/{total} exercises completed")
        ...
        <article className="writing-card">
          ...
          <div className="writing-meta">
            <span>{getUiLabel("writing.meta.level", "Level")} {exercise.level}</span>
            <span>{getUiLabel("writing.meta.best", "Best")}: {bestScore}%</span>
            <span>{getUiLabel("writing.meta.current", "Current")}: {currentLevel}</span>
            <span>{getUiLabel("writing.meta.attempts", "Attempts")}: {attempts}</span>
          </div>
          ...
          <textarea
            ...
            placeholder={getUiLabel("writing.answer_placeholder", "Write your answer...")}
          />
          ...
          <div className="writing-tip">
            {getUiLabel("writing.tip", "Tip")}: {exercise.tip}
          </div>
