...
function pickRound(level, usedIds) {
...
export default function Games({ setCurrentView, color = "#122987" }) {
  ...
  const advance = (nextScore, nextCombo, nextLevel, nextCorrect, nextUsed) => {
  ...
  return (
    <section className="games-shell" style={{ "--games-theme": color }}>
      <header className="games-head">
        ...
        <div>
          <div className="games-kicker">ORIGINAL GAME MODE</div>
          <h1>Speed Context</h1>
        </div>
      </header>
      ...
      {stage === "home" && (
        <article className="games-home-card">
          ...
          <p>Quanto maior o combo, maior a pontuação. Errou, combo zera e a dificuldade reduz.</p>

          <div className="games-stats">
            <span>Recorde: {record.high_score}</span>
            <span>Melhor combo: {record.best_combo}</span>
            <span>Sessoes: {record.total_sessions}</span>
          </div>
      ...
      {stage === "result" && (
        <article className="games-result-card">
          ...
          <p>Pontos: <strong>{score}</strong></p>
          <p>Acertos: <strong>{sessionCorrect}</strong> de {SESSION_ROUNDS}</p>
          <p>Melhor combo da sessão: <strong>{bestComboRun}</strong></p>
          <button type="button" className="games-start-btn" onClick={startGame}>
            Jogar novamente
          </button>
        </article>
