...

  {
    ...
    lessons: [
...

      {
        ...
        prompt: "OuÃ§a em velocidades diferentes e selecione o numero correto.",
...
function buildQuestionFeedback(question, isCorrect, submittedValue) {
  ...
  const problemFragment =
    ...
    errorType === "ordem"
      ...
      ? submittedValue || "A estrutura da frase ficou fora da ordem esperada."

      : missingTokens[0]

        ? `Faltou ou foi trocado o trecho "${missingTokens[0]}".`

        : unexpectedTokens[0]

          ? `O trecho "${unexpectedTokens[0]}" nao encaixa neste contexto.`

          : submittedValue || "A resposta nao seguiu o padrao esperado.";

  const explanationByType = {

    ordem: `A ideia principal esta correta, mas a ordem das palavras precisa seguir o padrao de ${conceptLabel}.`,

    lexical: `O significado central mudou ou ficou incompleto. Revise a palavra-chave de ${conceptLabel}.`,

    gramatical: `A estrutura gramatical nao bate com o padrao esperado para ${conceptLabel}.`,
  ...
  const extraExample =
    ...
    `Exemplo: ${answerSource || "Use the target structure in a short sentence."}`;
  ...
  if (isCorrect) {
    ...
    return {
      ...
      microRule: `Conceito: ${conceptLabel}`,

      errorTypeLabel: "dominio",

      explanation: `Voce aplicou corretamente o padrao de ${conceptLabel}.`,

      problemFragment: answerSource,
  ...
  return {
    ...
    microRule: `Reforce: ${conceptLabel}`,
...
function buildReviewUnits(units, conceptScores, recurringErrors = {}) {
  ...
  const dueReviews = Object.entries(conceptScores || {})
    ...
    .map((item, index) => ({
      ...
      title: `Review: ${formatConceptLabel(item.conceptKey)}`,

      concept: formatConceptLabel(item.conceptKey),

      summary: "Short targeted review based on weak performance and recurring errors.",

      focusSkills: ["review", "accuracy", "recovery"],

      guidePoints: [

        "This review was inserted automatically because the concept is unstable.",

        "Focus on the exact pattern you missed before moving on.",

      ],

      masterySignal: "You answer correctly twice in a row after revisiting the weak point.",
      ...
      lessons: [

        {
          ...
          prompt: `Quick review: which option best matches ${formatConceptLabel(item.conceptKey)}?`,
      ...
      {phase === "lesson" ? (
        <div className="grammar-lesson-body">
          ...
          <div className="grammar-audio-row">
            ...
            <p>{step.supportingCopy || "OuÃ§a o tema e responda as atividades abaixo"}</p>
          </div>
          {questionNode}
        </div>
      ) : null}
      ...
      {phase === "lesson" && feedback ? (
        <div className={`grammar-feedback-bar is-${feedback.tone}`}>
          <div className="grammar-feedback-copy">
            <strong>{feedback.title}</strong>
            <p>{feedback.summary}</p>
            {feedback.errorTypeLabel ? (
              <div className={`grammar-feedback-badge is-${feedback.errorTypeLabel}`}>
                Tipo de erro: {feedback.errorTypeLabel}
              </div>
            ) : null}
            {feedback.explanation ? <span>{feedback.explanation}</span> : null}
            {feedback.problemFragment ? (
              <div className="grammar-feedback-fragment">Trecho problematico: {feedback.problemFragment}</div>
            ) : null}
            {feedback.correctAnswer ? <span>Resposta esperada: {feedback.correctAnswer}</span> : null}
            {feedback.extraExample ? (
              <div className="grammar-feedback-example">Exemplo curto: {feedback.extraExample}</div>
            ) : null}
...
export default function GrammarPage({ setCurrentView, color = "#58cc02" }) {
  ...
  const reviewUnits = useMemo(() => buildReviewUnits([], conceptScores, recurringErrors), [conceptScores, recurringErrors]);

  const reviewUnit = reviewUnits[0] || null;

  const availableUnits = useMemo(
    () => buildReviewUnits(units, conceptScores, recurringErrors).map((unit) => enrichUnit(unit)),
    [units, conceptScores, recurringErrors]
  );
  ...
  const handleCompleteUnit = async (payload) => {
    ...
    const nextReviewUnit = buildReviewUnits([], nextConceptScores, nextRecurringErrors)[0] || null;
    ...
    <section className="grammar-page-shell" style={themeVars(color)}>
      <div className="grammar-header-card">
        ...
        <div className="grammar-header-content">
          <div className="grammar-header-kicker">{currentLevel} â€¢ TRILHA DE GRAMMAR</div>
